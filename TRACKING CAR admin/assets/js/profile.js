...
class ProfileManager {
  ...
  updateProfileDisplay() {
    ...
    document.getElementById("lieuAffectation").textContent =
      admin.lieuAffectation || "-";
    ...
    document.getElementById("profileRole").textContent = this.getRoleText(
      admin.role
    );
    document.getElementById("profileLegion").textContent = this.getLegionText(
      admin.legion
    );
    ...
    document.getElementById("accountCreated").textContent = admin.createdAt
      ? (admin.createdAt.seconds
          ? new Date(admin.createdAt.seconds * 1000)
  ...
  toggleEditMode(editing) {
    ...
    if (editing) {
      ...
      document.getElementById("editMatricule").value =
        this.adminData.matricule || "";
      document.getElementById("editLieuAffectation").value =
        this.adminData.lieuAffectation || "";
    } else {
  ...
  async loadLoginHistory() {
    try {
      ...
      loginHistory.innerHTML = snapshot.docs
        .map((doc) => {
          const log = doc.data();
          return `
  ...
  async loadUserStats() {
    try {
      ...
      document.getElementById("monthlyLogins").textContent =
        monthlyLoginsSnapshot.size;
      document.getElementById("totalActions").textContent =
        allActionsSnapshot.size;
      if (!lastActionSnapshot.empty) {
        const lastAction = lastActionSnapshot.docs[0].data();
        document.getElementById("lastAction").textContent =
          lastAction.timestamp && lastAction.timestamp.seconds
            ? new Date(lastAction.timestamp.seconds * 1000).toLocaleString(
                "fr-FR"
              )
            : "-";
      } else {
