...
class UsersManager {
  ...
  async loadUsers() {
    try {
      ...
      // Charger les infos utilisateurs complémentaires
      let usersData = {};
      try {
        ...
        console.warn("Collection 'users' non disponible:", error);
      }
      ...
      checks.forEach((c) => {
        ...
        if (!userMap.has(id)) {
          ...
          userMap.set(id, {
            id,
            displayName: userInfo.displayName || c.user_name || "",
            firstName: userInfo.firstName || c.firstName || "",
            lastName: userInfo.lastName || c.lastName || "",
            email: userInfo.email || c.user_email || "",
            matricule: userInfo.matricule || c.matricule || "",
            lieuAffectation:
              userInfo.lieuAffectation || c.lieuAffectation || "",
            createdAt: c.check_date?.seconds
              ? new Date(c.check_date.seconds * 1000)
              : null,
            lastActive: c.check_date?.seconds
              ? new Date(c.check_date.seconds * 1000)
              : null,
            checks: [],
            legion: c.legion || userInfo.legion || null,
            deviceInfo: userInfo.deviceInfo || c.deviceInfo || null,
        ...
        const checkDate = c.check_date?.seconds
          ? new Date(c.check_date.seconds * 1000)
          : null;
        ...
        // Mettre à jour les infos personnalisées si présentes
        if (c.firstName) user.firstName = c.firstName;
        if (c.lastName) user.lastName = c.lastName;
        if (c.matricule) user.matricule = c.matricule;
        if (c.lieuAffectation) user.lieuAffectation = c.lieuAffectation;
        if (c.deviceInfo) user.deviceInfo = c.deviceInfo;
      });
  ...
  updateStats() {
    ...
    const activeUsers = this.allUsers.filter(
      (user) => user.lastActive && user.lastActive >= oneWeekAgo
    ).length;
    const usersWithDetections = this.allUsers.filter(
      (user) => (user.checks?.length || 0) > 0
    ).length;
    const newUsersThisMonth = this.allUsers.filter(
      (user) => user.createdAt && user.createdAt >= oneMonthAgo
    ).length;

    document.getElementById("totalUsers").textContent = totalUsers;
    document.getElementById("activeUsers").textContent = activeUsers;
    document.getElementById("usersWithDetections").textContent =
      usersWithDetections;
  ...
  applyFilters() {
    this.filteredUsers = this.allUsers.filter((user) => {
      ...
      if (this.filters.search) {
        ...
        const matchFields = [user.displayName, user.email, user.id]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
  ...
  displayUsers() {
    ...
    container.innerHTML = usersToShow
      .map((user, index) => this.createUserCard(user, startIndex + index))
      .join("");
  ...
  createUserCard(user, index) {
    ...
    const stolenDetections = user.checks.filter(
      (c) => c.result === "stolen"
    ).length;
    ...
    const platform = user.deviceInfo?.platform || user.platform;
    const platformIcon =
      platform === "android"
        ? "fab fa-android"
        : platform === "ios"
            ...
            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all duration-300 bg-white slide-in-right" 
                ...
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            ${
                              user.matricule
                                ? `<p class="text-xs text-gray-500">Matricule: ${user.matricule}</p>`
                                : ""
                            }
                            <div class="flex items-center space-x-2 mt-1">
                                <span class="status-badge bg-${
                                  activityStatus.color
                                }-100 text-${activityStatus.color}-800 text-xs">
                                    <i class="fas fa-${
                                      activityStatus.icon
                                    } mr-1"></i>
                                    ${activityStatus.text}
                    ...
                    <div class="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div>
                            <div class="text-lg font-bold text-green-600">${
                              detectionCount - stolenDetections
                            }</div>
                ...
                <div class="space-y-2 text-sm text-gray-600 mb-4">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">${TrackingCarUtils.formatDate(
                          user.lastActive,
                          {
                            month: "short",
                            day: "numeric",
                ...
                <div class="flex justify-end space-x-2">
                    <button onclick="usersManager.viewUserProfile('${
                      user.id
                    }')" 
                ...
                <div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                <span class="text-gray-600">Lieu d'affectation:</span>
                                <span class="ml-2 font-medium">${
                                  user.lieuAffectation || "Non défini"
                                }</span>
                            </div>
                            <div>
                              ...
                              .map(
                                ...
                                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                                  ...
                                  detection.license_plate
                                        ...
                                        <div class="text-xs text-gray-500">${TrackingCarUtils.formatDate(
                                          detection.timestamp
                                        )}</div>
                                    </div>
                                    <span class="status-badge ${
                                      detection.result === "stolen"
  ...
  viewUserDetections(userId) {
    ...
    const user = this.allUsers.find((u) => u.id === userId);
    if (user) {
      window.location.href = `../detections/list.html?user=${encodeURIComponent(
        user.email || user.id
      )}`;
    }
  }
  ...
  async exportUserData(userId) {
    ...
    try {
      ...
      const userData = {
        ...
        "Première connexion": TrackingCarUtils.formatDate(
          user.createdAt || user.lastLoginAt
        ),
        ...
        "Véhicules volés détectés": userDetections.filter(
          (d) => d.result_data?.result === "stolen"
        ).length,
      ...
      link.setAttribute(
        "download",
        `utilisateur_${user.id}_${new Date().toISOString().split("T")[0]}.csv`
      );
      ...
      TrackingCarUtils.showNotification(
        "Export utilisateur réalisé avec succès",
        "success"
      );
    } catch (error) {
    ...
    try {
      ...
      const dataToExport = this.filteredUsers.map((user) => {
        ...
        return {
          ...
          "Véhicules propres": userDetections.filter(
            (d) => d.result_data?.result !== "stolen"
          ).length,
          Statut: this.isUserActive(user) ? "Actif" : "Inactif",
        };
